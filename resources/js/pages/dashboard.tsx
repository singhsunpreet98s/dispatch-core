...
interface CampaignDetail {
    ...
    email_config?: CampaignEmailConfig;
}
...
const fmtDateRange = (from: string, to: string) => {
    const f = (d: string) => new Intl.DateTimeFormat('en-US', { month: 'short', day: 'numeric', year: 'numeric' }).format(new Date(d + 'T00:00:00'));
...
function StatCard({
    ...
    return (
        <Card className="flex flex-col overflow-hidden">
            <CardHeader className="flex flex-row items-start justify-between space-y-0 px-5 pt-4 pb-1">
...
function ChartTooltip({ active, payload }: { active?: boolean; payload?: any[] }) {
    ...
    return (
        <div className="border-border bg-popover min-w-[170px] rounded-lg border px-3 py-2.5 shadow-lg">
            {label && <p className="border-border text-muted-foreground mb-2 border-b pb-1.5 text-[11px] font-semibold">{label}</p>}
...
function EmailAreaChart({ data }: { data: DailyStatPoint[] }) {
    ...
    if (chartData.length === 0) {
        return <div className="text-muted-foreground flex items-center justify-center py-16 text-sm">No daily data available for this period.</div>;
    }
    ...
    return (
        <Sheet open={open} onOpenChange={onOpenChange}>
            <SheetContent className="w-full overflow-y-auto sm:max-w-xl">
                ...
                {!loading && !data && <p className="text-muted-foreground py-8 text-center text-sm">Failed to load campaign details.</p>}
                {!loading && data && (
                    <div className="space-y-5">
                        <section>
                            <p className="text-muted-foreground mb-1 text-xs font-semibold tracking-wider uppercase">Overview</p>
                            <div className="divide-y rounded-lg border">
                                <div className="px-4">
                                    <DetailRow label="Campaign ID" value={<span className="font-mono text-xs">{data.detail.id}</span>} />
                                </div>
                                <div className="px-4">
                                    <DetailRow
                                        label="Status"
                                        value={
                                            <Badge variant={statusVariant(data.detail.status)}>
                                                {statusLabel[data.detail.status] ?? data.detail.status}
                                            </Badge>
                                        }
                                    />
                                </div>
                                <div className="px-4">
                                    <DetailRow label="Send Date" value={formatDate(data.detail.send_at)} />
                                </div>
                                <div className="px-4">
                                    <DetailRow label="Created" value={formatDate(data.detail.created_at)} />
                                </div>
                                <div className="px-4">
                                    <DetailRow label="Last Updated" value={formatDate(data.detail.updated_at)} />
                                </div>
                            </div>
                        </section>
                        {data.detail.email_config && (
                            <section>
                                <p className="text-muted-foreground mb-1 text-xs font-semibold tracking-wider uppercase">Email Configuration</p>
                                <div className="divide-y rounded-lg border">
                                    <div className="px-4">
                                        <DetailRow label="Subject" value={data.detail.email_config.subject} />
                                    </div>
                                    {(data.sender_name || data.detail.email_config.sender_id) && (
                                        <div className="px-4">
                                            <DetailRow label="Sender" value={data.sender_name ?? String(data.detail.email_config.sender_id)} />
                                        </div>
                                    )}
                                    {data.detail.email_config.ip_pool && (
                                        <div className="px-4">
                                            <DetailRow label="IP Pool" value={data.detail.email_config.ip_pool} />
                                        </div>
                                    )}
                                    <div className="px-4">
                                        <DetailRow label="Editor" value={data.detail.email_config.editor} />
                                    </div>
                                </div>
                            </section>
                        )}
                        <section>
                            <p className="text-muted-foreground mb-1 text-xs font-semibold tracking-wider uppercase">Delivery Stats</p>
            ...
            <CardHeader className="pb-3">
                <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                    <CardTitle className="flex items-center gap-2 text-base font-semibold text-amber-700 dark:text-amber-400">
                        ...
                        Action Required
                        <span className="rounded-full bg-amber-500 px-2 py-0.5 text-xs leading-none font-bold text-white">{items.length}</span>
                    </CardTitle>
                    <div className="flex flex-wrap gap-1.5">
                        <button
                            onClick={() => setActiveType('all')}
                            className={`rounded-full border px-2.5 py-0.5 text-xs font-medium transition-colors ${activeType === 'all' ? 'border-amber-500 bg-amber-500 text-white' : 'border-amber-300 text-amber-600 hover:bg-amber-100 dark:border-amber-700 dark:text-amber-400 dark:hover:bg-amber-900/30'}`}
                        >
                            All ({items.length})
                        </button>
                        {types.map((t) => {
                            ...
                            return (
                                <button
                                    ...
                                    className={`rounded-full border px-2.5 py-0.5 text-xs font-medium transition-colors ${activeType === t ? 'border-amber-500 bg-amber-500 text-white' : 'border-amber-300 text-amber-600 hover:bg-amber-100 dark:border-amber-700 dark:text-amber-400 dark:hover:bg-amber-900/30'}`}
                                >
                                    {TYPE_LABELS[t]} ({count})
                                </button>
                            );
            ...
            <CardContent className="space-y-2 pt-0">
                {visible.map((item) => (
                    <div
                        ...
                        className="flex flex-col gap-2 rounded-lg border border-amber-100 bg-white px-4 py-3 sm:flex-row sm:items-center sm:gap-4 dark:border-amber-900/30 dark:bg-amber-950/20"
                    >
                        <span className="inline-flex w-fit items-center gap-1 rounded-full bg-amber-100 px-2 py-0.5 text-[10px] font-semibold tracking-wide text-amber-700 uppercase dark:bg-amber-900/40 dark:text-amber-300">
                        ...
                        {item.type === 'leave' && (
                            <div className="flex shrink-0 gap-2">
                                <Button
                                    size="sm"
                                    variant="outline"
                                    className="h-7 gap-1 border-green-300 text-green-700 hover:bg-green-50 hover:text-green-800 dark:border-green-700 dark:text-green-400 dark:hover:bg-green-900/20"
                                    onClick={() => router.patch(route('attendance.leave.approve', item.id), {}, { preserveScroll: true })}
                                >
                                ...
                                <Button
                                    size="sm"
                                    variant="outline"
                                    className="h-7 gap-1 border-red-300 text-red-600 hover:bg-red-50 hover:text-red-700 dark:border-red-700 dark:text-red-400 dark:hover:bg-red-900/20"
                                    onClick={() => router.patch(route('attendance.leave.reject', item.id), {}, { preserveScroll: true })}
                                >
...
export default function Dashboard({ stats, dailyStats, recentCampaigns, actionItems, filters, users, selectedUser, isAdmin }: Props) {
    ...
    const activePreset = filters.date_to === today() ? (PRESETS.find((p) => subDays(p.days) === filters.date_from)?.label ?? null) : null;
    ...
    const statCards = [
        {
            ...
            icon: Mail,
            color: C.indigo,
            sparkKey: 'requests' as keyof DailyStatPoint,
        },
        {
            title: 'Delivered',
            value: fmt(stats.delivered),
            sub: `${stats.delivery_rate}% rate`,
            icon: CheckCircle2,
            color: C.emerald,
            sparkKey: 'delivered' as keyof DailyStatPoint,
        },
        {
            title: 'Undelivered',
            ...
            icon: TrendingDown,
            color: C.rose,
            sparkKey: 'undelivered' as keyof DailyStatPoint,
        },
        {
            title: 'Bounces',
            ...
            icon: AlertTriangle,
            color: C.amber,
            sparkKey: 'bounces' as keyof DailyStatPoint,
        },
        {
            title: 'Unique Opens',
            ...
            icon: Eye,
            color: C.violet,
            sparkKey: 'unique_opens' as keyof DailyStatPoint,
        },
        {
            title: 'Unique Clicks',
            value: fmt(stats.unique_clicks),
            sub: `${stats.click_rate}% CTR`,
            icon: MousePointerClick,
            color: C.cyan,
            sparkKey: 'unique_clicks' as keyof DailyStatPoint,
        },
        {
            title: 'Spam Reports',
            ...
            icon: TrendingUp,
            color: C.orange,
            sparkKey: 'spam_reports' as keyof DailyStatPoint,
        },
        {
            title: 'Unsubscribes',
            ...
            icon: UserMinus,
            color: C.slate,
            sparkKey: 'unsubscribes' as keyof DailyStatPoint,
        },
    ];
                    ...
                    <div className="flex flex-col gap-1 sm:flex-row sm:items-start sm:justify-between">
                        <div>
                            ...
                            <p className="text-muted-foreground text-sm">
                                {isAdmin
                                    ? `${fmtLabel(filters.date_from)} – ${fmtLabel(filters.date_to)} · Portal`
                                    : 'Your campaign statistics · Portal'}
                            </p>
                            {selectedUser && (
                                <p className="mt-1 flex items-center gap-1.5 text-xs text-indigo-600 dark:text-indigo-400">
                                    ...
                                    <button
                                        onClick={() => navigate(filters.date_from, filters.date_to, null)}
                        ...
                        {isAdmin && (
                            <UserFilter
                                ...
                                onChange={(uid) => navigate(filters.date_from, filters.date_to, uid)}
                            />
                        )}
                    </div>
                ...
                {isAdmin && !selectedUser && (
                    <Card>
                        <CardHeader className="pb-2">
                            ...
                            <p className="text-muted-foreground text-xs">Daily breakdown — sent, delivered, undelivered, and bounces</p>
                        </CardHeader>
                                    ...
                                    <tbody className="divide-y">
                                        {recentCampaigns.map((campaign) => (
                                            <tr key={campaign.id} className="hover:bg-muted/30 transition-colors">
                                                ...
                                                <td className="text-muted-foreground px-6 py-4">{formatDate(campaign.send_at)}</td>
                                                <td className="text-muted-foreground px-6 py-4">{formatDate(campaign.updated_at)}</td>
